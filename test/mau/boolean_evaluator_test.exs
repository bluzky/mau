  ...
  describe "Comparison Operation Evaluation" do
    ...
    test "evaluates string equality" do
      assert {:ok, "true"} =
               Renderer.render_node(
                 {:expression,
                  [
                    {:binary_op, ["==", {:literal, ["hello"], []}, {:literal, ["hello"], []}], []}
                  ], []},
                 %{}
               )
      ...
      assert {:ok, "false"} =
               Renderer.render_node(
                 {:expression,
                  [
                    {:binary_op, ["==", {:literal, ["hello"], []}, {:literal, ["world"], []}], []}
                  ], []},
                 %{}
               )
    end
    ...
    test "evaluates float relational operations" do
      assert {:ok, "true"} =
               Renderer.render_node(
                 {:expression,
                  [{:binary_op, [">", {:literal, [3.14], []}, {:literal, [2.71], []}], []}], []},
                 %{}
               )
      ...
      assert {:ok, "false"} =
               Renderer.render_node(
                 {:expression,
                  [{:binary_op, [">", {:literal, [2.71], []}, {:literal, [3.14], []}], []}], []},
                 %{}
               )
    end
    ...
    test "evaluates string relational operations" do
      assert {:ok, "true"} =
               Renderer.render_node(
                 {:expression,
                  [
                    {:binary_op, ["<", {:literal, ["apple"], []}, {:literal, ["banana"], []}], []}
                  ], []},
                 %{}
               )
      ...
      assert {:ok, "true"} =
               Renderer.render_node(
                 {:expression,
                  [
                    {:binary_op, [">=", {:literal, ["apple"], []}, {:literal, ["apple"], []}], []}
                  ], []},
                 %{}
               )
    end
    ...
    test "handles comparison with variables" do
      ...
      assert {:ok, "true"} =
               Renderer.render_node(
                 {:expression,
                  [{:binary_op, [">", {:variable, ["age"], []}, {:literal, [18], []}], []}], []},
                 context
               )
      ...
      assert {:ok, "true"} =
               Renderer.render_node(
                 {:expression,
                  [
                    {:binary_op, ["==", {:variable, ["name"], []}, {:literal, ["Alice"], []}], []}
                  ], []},
                 context
               )
    end
    ...
    test "handles comparison with nil values" do
      ...
      assert {:ok, "true"} =
               Renderer.render_node(
                 {:expression,
                  [{:binary_op, ["==", {:variable, ["value"], []}, {:literal, [nil], []}], []}],
                  []},
                 context
               )
      ...
      assert {:ok, "true"} =
               Renderer.render_node(
                 {:expression,
                  [
                    {:binary_op, ["==", {:variable, ["undefined"], []}, {:literal, [nil], []}],
                     []}
                  ], []},
                 context
               )
    ...
    test "returns error for unsupported relational operations" do
      assert {:error, %Mau.Error{type: :runtime}} =
               Renderer.render_node(
                 {:expression,
                  [{:binary_op, [">", {:literal, ["hello"], []}, {:literal, [5], []}], []}], []},
                 %{}
               )
      ...
      assert {:error, %Mau.Error{type: :runtime}} =
               Renderer.render_node(
                 {:expression,
                  [{:binary_op, ["<", {:literal, [5], []}, {:literal, [true], []}], []}], []},
                 %{}
               )
    end
  end
  ...
  describe "Logical Operation Evaluation" do
    test "evaluates AND operations with short-circuiting" do
      ...
      assert {:ok, "true"} =
               Renderer.render_node(
                 {:expression,
                  [{:logical_op, ["and", {:literal, [true], []}, {:literal, [true], []}], []}],
                  []},
                 %{}
               )
      ...
      assert {:ok, "false"} =
               Renderer.render_node(
                 {:expression,
                  [{:logical_op, ["and", {:literal, [true], []}, {:literal, [false], []}], []}],
                  []},
                 %{}
               )
      ...
      assert {:ok, "false"} =
               Renderer.render_node(
                 {:expression,
                  [{:logical_op, ["and", {:literal, [false], []}, {:literal, [false], []}], []}],
                  []},
                 %{}
               )
    end
    ...
    test "evaluates OR operations with short-circuiting" do
      ...
      assert {:ok, "true"} =
               Renderer.render_node(
                 {:expression,
                  [{:logical_op, ["or", {:literal, [true], []}, {:literal, [true], []}], []}],
                  []},
                 %{}
               )
      ...
      assert {:ok, "true"} =
               Renderer.render_node(
                 {:expression,
                  [{:logical_op, ["or", {:literal, [true], []}, {:literal, [false], []}], []}],
                  []},
                 %{}
               )
      ...
      assert {:ok, "true"} =
               Renderer.render_node(
                 {:expression,
                  [{:logical_op, ["or", {:literal, [false], []}, {:literal, [true], []}], []}],
                  []},
                 %{}
               )
      ...
      assert {:ok, "false"} =
               Renderer.render_node(
                 {:expression,
                  [{:logical_op, ["or", {:literal, [false], []}, {:literal, [false], []}], []}],
                  []},
                 %{}
               )
    end
    ...
    test "handles truthiness evaluation correctly" do
      ...
      assert {:ok, "true"} =
               Renderer.render_node(
                 {:expression,
                  [{:logical_op, ["and", {:literal, [1], []}, {:literal, [true], []}], []}], []},
                 %{}
               )
      ...
      assert {:ok, "true"} =
               Renderer.render_node(
                 {:expression,
                  [{:logical_op, ["and", {:literal, ["hello"], []}, {:literal, [true], []}], []}],
                  []},
                 %{}
               )
      ...
      assert {:ok, "true"} =
               Renderer.render_node(
                 {:expression,
                  [{:logical_op, ["and", {:literal, [[1, 2]], []}, {:literal, [true], []}], []}],
                  []},
                 %{}
               )
      ...
      assert {:ok, "false"} =
               Renderer.render_node(
                 {:expression,
                  [{:logical_op, ["and", {:literal, [0], []}, {:literal, [true], []}], []}], []},
                 %{}
               )
      ...
      assert {:ok, "false"} =
               Renderer.render_node(
                 {:expression,
                  [{:logical_op, ["and", {:literal, [""], []}, {:literal, [true], []}], []}], []},
                 %{}
               )
      ...
      assert {:ok, "false"} =
               Renderer.render_node(
                 {:expression,
                  [{:logical_op, ["and", {:literal, [nil], []}, {:literal, [true], []}], []}],
                  []},
                 %{}
               )
    ...
    test "handles float zero correctly in truthiness" do
      assert {:ok, "false"} =
               Renderer.render_node(
                 {:expression,
                  [{:logical_op, ["and", {:literal, [0.0], []}, {:literal, [true], []}], []}],
                  []},
                 %{}
               )
      ...
      assert {:ok, "true"} =
               Renderer.render_node(
                 {:expression,
                  [{:logical_op, ["and", {:literal, [0.1], []}, {:literal, [true], []}], []}],
                  []},
                 %{}
               )
    end
      ...
      assert {:ok, "false"} =
               Renderer.render_node(
                 {:expression,
                  [
                    {:logical_op,
                     ["and", {:variable, ["is_admin"], []}, {:variable, ["is_active"], []}], []}
                  ], []},
                 context
               )
      ...
      assert {:ok, "true"} =
               Renderer.render_node(
                 {:expression,
                  [
                    {:logical_op,
                     ["or", {:variable, ["is_admin"], []}, {:variable, ["is_active"], []}], []}
                  ], []},
                 context
               )
    end
